...
/// REMOVE ME
///
/// The serial code is only here temporarily until we can setup a ring buffer
/// PV interface that can be used by another VM to export serial data.
///
////////////////////////////////////////////////////////////////////////////////
...
constexpr const uint8_t fifo_control_dma_mode_select = 1U << 3;

constexpr const uint8_t line_status_data_ready = 1U << 0;
constexpr const uint8_t line_status_overrun_error = 1U << 1;
constexpr const uint8_t line_status_parity_error = 1U << 2;
constexpr const uint8_t line_status_framing_error = 1U << 3;
constexpr const uint8_t line_status_break_interrupt = 1U << 4;
constexpr const uint8_t line_status_empty_transmitter = 1U << 5;
constexpr const uint8_t line_status_empty_data = 1U << 6;
constexpr const uint8_t line_status_recieved_fifo_error = 1U << 7;
...
public:
    ...
    /// @ensures none
    ///
    serial_x64(port_type port = DEFAULT_COM_PORT) noexcept;
    ...
    /// @ensures ret != nullptr
    ///
    static serial_x64 *instance() noexcept;
    ...
    ///
    void write(const char *str, uint64_t len) noexcept;
...
public:
    ...
    serial_x64(serial_x64 &&) noexcept = default;
    serial_x64 &operator=(serial_x64 &&) noexcept = default;
    serial_x64(const serial_x64 &) = delete;
    serial_x64 &operator=(const serial_x64 &) = delete;
};
